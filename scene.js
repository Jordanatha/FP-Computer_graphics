...
function initScene() {
    ...
    skyboxMaterial.reflectionTexture = new BABYLON.CubeTexture("sky/sky/TropicalSunnyDay", scene);
...
function playMusic(){
    var music = new BABYLON.Sound("Music", "music/music.wav", scene, null, { loop: false, autoplay: true, playbackRate: 0.8 });
    setTimeout(
        function(){
            $("#myModal").modal();
            scene = new BABYLON.Scene(engine);
...
function initGame() {
    ...
    //create torus where the beat will be killed
...
function tapBeat(ending){
    for (var i=0; i<beats.length; i++){
        var beatt = beats[i];
    // Check if the beat is on the good lane
        if (beatt.position.x === ending.position.x) {

            // Check if the beat is ON the ending +8 or -8
            var diffSup = ending.position.z + 5;
            var diffInf = ending.position.z - 5;
            ...
            if(beatt.position.z < diffInf){
                miss++;
                document.getElementById("miss").innerHTML = miss;
                document.getElementById("miss2").innerHTML = miss;
            }
        }
    }
    return null;
